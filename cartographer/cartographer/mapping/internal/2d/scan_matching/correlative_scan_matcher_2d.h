...
struct SearchParameters {
  ...
  int num_angular_perturbations;          // 角度的搜索个数(单方向)
  double angular_perturbation_step_size;  // // 角度的搜索步长
  double resolution;
  int num_scans;  // 角度的搜索个数 2*num_angular_perturbations+1

  // 一个搜索角度对应一个旋转点云
  // 每一个旋转点云的搜索范围, 经过ShrinkToFit()之后就不一样了
