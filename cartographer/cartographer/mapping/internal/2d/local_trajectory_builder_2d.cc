...
// 重力对齐, 提取地面points, 体素滤波
sensor::RangeData
LocalTrajectoryBuilder2D::TransformToGravityAlignedFrameAndFilter(
    const transform::Rigid3f&
        transform_to_gravity_aligned_frame,  // odom to base
    const sensor::RangeData& range_data) const {
  // 重力对齐的同时, 还将点云从odom系转换到base系下(重力对齐后base)
...
LocalTrajectoryBuilder2D::AddRangeData(
  ...
  // 1.多激光传感器数据基于时间戳同步融合 (只有一个传感器的时候,可以直接忽略)
  ...
  // 2.预测每个point对应的pose (base_link在odom下的pose), 用于畸变校准
...
LocalTrajectoryBuilder2D::AddAccumulatedRangeData(
  ...
  // 2.构造submap
  sensor::RangeData range_data_in_local =  // in_local : in_odom
