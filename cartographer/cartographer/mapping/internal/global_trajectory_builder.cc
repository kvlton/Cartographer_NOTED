...
class GlobalTrajectoryBuilder : public mapping::TrajectoryBuilderInterface {
 public:
  ...
  void AddSensorData(
    ...
    // 2.后端优化, 闭环检测
    kLocalSlamMatchingResults->Increment(); // 帧间匹配次数
    std::unique_ptr<InsertionResult> insertion_result;
    if (matching_result->insertion_result != nullptr) { // 运动滤波, 筛选关键帧
      kLocalSlamInsertionResults->Increment(); // 插入到子图的帧数
    ...
    // 3.回调 MapBuilderBridge::OnLocalSlamResult()
